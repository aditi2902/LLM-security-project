...
# All prompts are automatically intercepted by TrustLens`;
  ...
  const bypassSeries = trend.map(t => t.bypass_rate);

  return (
    <div className="animate-in">
      <header className="page-header">
        <h1 className="page-title glitch" data-text="ANALYTICS">ANALYTICS</h1>
        <p className="page-subtitle">
          Real-time security metrics, bypass trends, and attack category breakdown across all TrustLens traffic.
        </p>
      </header>
      ...
      <div className="analytics-top-row">
        ...
        {/* Security Score */}
        <div className="glass-panel analytics-score-panel">
          <ScoreRing score={score} />
          <div className="divider" style={{ margin: '1.25rem 0' }} />
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', width: '100%' }}>
            {[
              { label: 'Bypass Rate', val: `${data?.bypass_rate ?? 0}%`, color: data?.bypass_rate > 20 ? 'var(--red)' : 'var(--green)' },
              { label: 'Block Rate',  val: `${data?.block_rate ?? 0}%`,  color: 'var(--primary)' },
            ].map(s => (
              <div key={s.label} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontFamily: 'var(--font-ui)', fontSize: '0.78rem', color: 'var(--text-muted)' }}>{s.label}</span>
                <span style={{ fontFamily: 'var(--font-display)', fontSize: '1rem', fontWeight: 700, color: s.color }}>{s.val}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="analytics-stat-grid">
          <StatCard icon={<Cpu size={22} />}        label="Queries Processed"   value={data?.total_queries ?? 0}  color="var(--primary)" />
          <StatCard icon={<AlertOctagon size={22} />} label="Ingress Blocked"   value={data?.blocked_ingress ?? 0} color="var(--red)" />
          <StatCard icon={<Shield size={22} />}     label="Attacks Fired"       value={data?.total_attacks ?? 0}  color="var(--cyan)" />
          <StatCard icon={<CheckCircle2 size={22} />} label="Attacks Blocked"   value={data?.blocked_count ?? 0}  color="var(--green)" />
        </div>
      </div>
      ...
      {trend.length > 0 && (
        <div className="glass-panel" style={{ padding: '1.5rem', marginTop: '1.5rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
            <div className="data-block-label" style={{ color: 'var(--primary)' }}>
              <BarChart2 size={14} style={{ display: 'inline', marginRight: '0.4rem' }} />
              Bypass Rate — Last 7 Days
            </div>
            <button onClick={load} style={{
              background: 'none', border: '1px solid rgba(0,229,255,0.2)', borderRadius: '6px',
              padding: '0.3rem 0.7rem', color: 'var(--primary)', fontSize: '0.75rem',
              fontFamily: 'var(--font-ui)', fontWeight: 700, cursor: 'pointer',
            }}>
              <Activity size={12} style={{ marginRight: '0.3rem' }} />Refresh
            </button>
          </div>
            ...
            {trend.map(t => (
              <div key={t.date} style={{ flex: 1, textAlign: 'center' }}>
                <div style={{ fontFamily: 'var(--font-display)', fontSize: '1.1rem', fontWeight: 800,
                  color: t.bypass_rate > 30 ? 'var(--red)' : 'var(--green)' }}>
                  {t.bypass_rate}%
                </div>
          ...
          <Sparkline data={bypassSeries} color="var(--primary)" height={52} />
        </div>
      )}
      ...
      {cats.length > 0 && (
        <div className="glass-panel" style={{ padding: '1.5rem', marginTop: '1.5rem' }}>
          <div className="data-block-label" style={{ color: 'var(--primary)', marginBottom: '1.25rem' }}>
            <Target size={14} style={{ display: 'inline', marginRight: '0.4rem' }} />
            Attack Category Breakdown
          </div>
          {cats.map(c => (
            <CategoryBar key={c.category}
              name={c.category} total={c.total}
              bypassed={c.bypassed} blocked={c.blocked}
              maxTotal={maxCat}
            />
          ))}
        </div>
      ...
      {/* ── Recent attacks ── */}
      {(data?.recent_attacks ?? []).length > 0 && (
        <div className="glass-panel" style={{ padding: '1.5rem', marginTop: '1.5rem' }}>
          <div className="data-block-label" style={{ color: 'var(--primary)', marginBottom: '1rem' }}>
            <History size={14} style={{ display: 'inline', marginRight: '0.4rem' }} />
            Recent Attacks
          </div>
          {data.recent_attacks.map((a, i) => (
            <div key={i} style={{
              display: 'flex', gap: '0.75rem', alignItems: 'flex-start',
              padding: '0.65rem 0', borderBottom: i < data.recent_attacks.length - 1 ? '1px solid rgba(255,255,255,0.04)' : 'none',
            }}>
              <span style={{
                flexShrink: 0, fontFamily: 'var(--font-mono)', fontSize: '0.65rem',
                color: a.status === 'BYPASSED' ? 'var(--red)' : 'var(--green)',
                background: a.status === 'BYPASSED' ? 'rgba(244,67,54,0.1)' : 'rgba(76,175,80,0.1)',
                border: `1px solid ${a.status === 'BYPASSED' ? 'rgba(244,67,54,0.3)' : 'rgba(76,175,80,0.3)'}`,
                borderRadius: '3px', padding: '0.1rem 0.4rem',
              }}>{a.status}</span>
              <span style={{
                fontFamily: 'var(--font-ui)', fontSize: '0.72rem', fontWeight: 600,
                color: 'var(--primary)', flexShrink: 0, minWidth: '120px',
              }}>{(a.category || 'UNKNOWN').replace(/_/g, ' ')}</span>
